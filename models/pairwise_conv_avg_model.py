...
class PairwiseConvAvgModel(UnetBackbone):
    ...
    def forward(self,
                ...
                target: torch.Tensor,
                compute_maps = True) -> torch.Tensor:
        """
        context: [B, L, C, H, W]
        context_out: [B, L, C, H, W]
        target: [B, C, H, W]
        maps: optional [B, M, H, W]
        """
        if compute_maps:
            residual = residual_noise_map_2d(target)
            variance = local_variance_map_2d(target)
            maps = torch.cat([residual, variance], dim=1)  # [B,3,H,W]
        else:
            maps = None
        # Concatenate maps to target
        if maps is not None and self.use_maps_in_target:
            target = torch.cat([target, maps], dim=1)

        # Concatenate maps to context if requested
        if maps is not None and self.use_maps_in_context:
            maps_exp = maps.unsqueeze(1).expand(-1, context.shape[1], -1, *maps.shape[2:])
            context = torch.cat([context, maps_exp], dim=2)

        return super().forward(context, context_out, target)
...
@dataclass(eq=False, repr=False)
class PairwiseConvAvgModelBlock2(DefaultUnetStageBlock):
    ...
    def __post_init__(self):
        ...
        conv_fn = getattr(nn, f'Conv{self.dim}d')
        self.combine_conv_target = Vmap(conv_fn(
            in_channels=2*self.channels,
            out_channels=self.channels,
            kernel_size=1,
            padding='same'
        ))
        self.combine_conv_context = Vmap(conv_fn(
            in_channels=2*self.channels,
            out_channels=self.channels,
            kernel_size=1,
            padding='same'
        ...
        # Optional learnable scaling for each map
    ...
    def forward(self, context: torch.Tensor, target: torch.Tensor):
        ...
        target: [B,C,H,W]
        """
        # --- Compute maps from target features dynamically ---
        res_map, var_map = self.compute_maps(target)
        res_ctx_map, var_ctx_map = self.compute_context_maps(context)


        # --- Target features ---
        target = self.target_conv(target)
        target = target * (1 + self.alpha_res * res_map)
        target = target * (1 + self.alpha_var * var_map)

        # --- Context features ---
        context = self.context_conv(context)
        res_map_ctx = res_ctx_map.expand_as(context)
        var_map_ctx = var_ctx_map.expand_as(context)
        context = context * (1 + self.alpha_res_ctx * res_map_ctx)
        context = context * (1 + self.alpha_var_ctx * var_map_ctx)

        # --- Standard pairwise conv ---
        context_target = torch.concat([context, target.unsqueeze(1).expand_as(context)], dim=2)
        target_update = self.combine_conv_target(context_target).mean(dim=1, keepdim=False)
        context_update = self.combine_conv_context(context_target)
