...
from typing import *
#from nn.noise_maps import residual_noise_map_2d, local_variance_map_2d
...
class PairwiseConvAvgModel(UnetBackbone):
    ...
    def forward(self,
                ...
                target: torch.Tensor,
                maps: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        context: [B, L, C, H, W]
...
if __name__ == '__main__':
    device = 'cpu'
    num_target_channels = 2
    num_maps = 2   
    num_context_channels = 5 + num_maps
    unet2d = PairwiseConvAvgModel(dim=2, stages=4, in_channels= num_target_channels + num_maps,
                                  out_channels=3, inner_channels=32, conv_layers_per_stage=2).to(device)
    unet2d.context_embedding = Vmap(
    nn.Conv2d(num_context_channels, 32, kernel_size=1, padding="same")
)
    ...
    # compute maps
    #residual = residual_noise_map_2d(target_in)
    #variance = local_variance_map_2d(target_in)

    #maps = torch.cat([residual, variance], dim=1)  # [B,3,H,W]
    #print(maps.shape)
    target_out = unet2d(context_in, context_out, target_in,maps=None)
    print(unet2d)
    print('2d ok')
